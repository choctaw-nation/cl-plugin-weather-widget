...
class Bootstrap_Icons {
	 ...
	 * @param bool   $wrapped Whether to wrap the SVG in a span with a default aria-hidden attribute for accessibility. Default is true.
	 * @return string The corresponding SVG icon.
	 */
	public function get_the_icon( string $icon, bool $wrapped = true ): string {
		if ( ! array_key_exists( strtolower( $icon ), $this->icons ) ) {
			_doing_it_wrong(
				__METHOD__,
				sprintf(
					/* translators: %s: The invalid weather condition. */
					esc_html__( 'Invalid weather condition "%s". Returning empty string.', 'choctaw-landing-weather-widget' ),
					esc_html( $icon )
				),
				'1.0.0'
			);
		...
		$svg = $this->icons[ strtolower( $icon ) ];
		if ( $wrapped ) {
			$svg = sprintf( '<span aria-hidden="true">%s</span>', $svg );
		}
		return $svg;
	}
